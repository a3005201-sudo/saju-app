...
import { ageWithKorean, withKoreanLabel } from '../../utils/ziwei-labels.ts'
...
function PalaceCell({ palace, daxianRange }: { palace: ZiweiPalace; daxianRange?: string }) {
  ...
  return (
    <div className="flex flex-col justify-between h-full p-2 text-base leading-tight">
      {/* 궁명 + 신궁 + 간지 */}
      <div>
        <div className="flex items-baseline justify-between gap-1">
          <span className="font-medium text-gray-800 dark:text-gray-100">
            {withKoreanLabel(palace.name)}
          ...
          {mainStars.length > 0 ? (
            mainStars.map(s => (
              <div key={s.name} className="font-hanja">
                <span>{withKoreanLabel(s.name)}</span>
                {s.brightness && (
                  <span className={`ml-0.5 ${brightnessColor(s.brightness)}`}>{withKoreanLabel(s.brightness)}</span>
                )}
                {s.siHua && (
                  <span className={`ml-0.5 ${siHuaColor(s.siHua)}`}>{withKoreanLabel(s.siHua)}</span>
                )}
              </div>
            ))
          ) : (
            <div className="text-gray-400 dark:text-gray-500">({withKoreanLabel('空宮')})</div>
          )}
        </div>
        ...
        {(luckyStars.length > 0 || shaStars.length > 0) && (
          <div className="mt-1 pt-1 border-t border-dashed border-gray-200 dark:border-gray-700">
            {luckyStars.length > 0 && (
              <div className="text-green-600 dark:text-green-400">{luckyStars.map(s => withKoreanLabel(s.name)).join(' ')}</div>
            )}
            {shaStars.length > 0 && (
              <div className="text-red-500 dark:text-red-400">{shaStars.map(s => withKoreanLabel(s.name)).join(' ')}</div>
            )}
          </div>
        )}
      </div>
      ...
      {/* 대한 나이 범위 */}
      {daxianRange && (
        <div className="text-sm text-gray-500 dark:text-gray-400 text-right mt-1">{daxianRange && ageWithKorean(daxianRange)}</div>
      )}
    </div>
  )
}
...
export default function MingPanGrid({ chart }: Props) {
  ...
  return (
    <div className="overflow-x-auto rounded-lg">
      <div
        className="grid grid-cols-4 grid-rows-4 border-t border-l border-indigo-200 dark:border-gray-600 min-w-[980px]"
      >
        {/* 12궁 셀 */}
        {DI_ZHI.split('').map(zhi => {
          ...
          return (
            <div
              key={zhi}
              className="border-r border-b border-indigo-200 dark:border-gray-600 min-h-[170px] bg-white/80 dark:bg-gray-900/40"
        ...
        <div
          className="border-r border-b border-indigo-200 dark:border-gray-600 flex flex-col items-center justify-center p-4 bg-indigo-50/70 dark:bg-gray-900/50"
          style={{ gridRow: '2 / 4', gridColumn: '2 / 4' }}
        >
          <div className="space-y-1 text-base text-gray-700 dark:text-gray-200 w-full max-w-[300px]">
            <div>
              <span className="text-indigo-600 dark:text-indigo-300 font-medium">{withKoreanLabel('陽曆')}:</span>{' '}
              {chart.solarYear}年 {chart.solarMonth}月 {chart.solarDay}日 {chart.hour}時 {chart.minute}分
            </div>
            <div>
              <span className="text-indigo-600 dark:text-indigo-300 font-medium">{withKoreanLabel('陰曆')}:</span>{' '}
            ...
            <div>
              <span className="text-indigo-600 dark:text-indigo-300 font-medium">{withKoreanLabel('性別')}:</span> {genderChar}
            </div>
            <div className="pt-1">
              <span className="text-indigo-600 dark:text-indigo-300 font-medium">{withKoreanLabel('年柱')}:</span>{' '}
              <span className="font-hanja">{chart.yearGan}{chart.yearZhi}</span>
            </div>
            <div>
              <span className="text-indigo-600 dark:text-indigo-300 font-medium">{withKoreanLabel('命宮')}:</span>{' '}
              <span className="font-hanja">{chart.palaces['命宮']?.ganZhi}</span>
            </div>
            <div>
              <span className="text-indigo-600 dark:text-indigo-300 font-medium">{withKoreanLabel('身宮')}:</span>{' '}
              {withKoreanLabel(shenPalaceName)} (<span className="font-hanja">{chart.shenGongZhi}</span>)
            </div>
            <div>
              <span className="text-indigo-600 dark:text-indigo-300 font-medium">{withKoreanLabel('五行局')}:</span> {chart.wuXingJu.name}
            </div>
            <div>
              <span className="text-indigo-600 dark:text-indigo-300 font-medium">대한 시작:</span> {chart.daXianStartAge}세
            </div>
          </div>
        </div>
      </div>
