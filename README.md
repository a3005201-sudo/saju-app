...
- **개발용 주소(로컬):** `http://localhost:5173/`
- **배포용 주소(실서비스):** `https://rath.github.io/orrery/`

### 중요한 원칙

- 로컬에서 코드가 바뀌어도 배포 주소는 자동으로 즉시 바뀌지 않습니다.
- **`main` 브랜치에 push** 해야 배포 주소에 반영됩니다.
- 이 저장소는 `.github/workflows/deploy.yml`로 **GitHub Pages 자동 배포**가 설정되어 있습니다.

### 반영 절차 (항상 이 순서)

1. 로컬 개발 서버에서 확인 (`http://localhost:5173/`)
2. 빌드 확인 (`bun run build` 또는 `npm run build`)
3. 커밋
4. `main` 브랜치로 push
5. GitHub Actions `Deploy to GitHub Pages` 완료 확인
6. 배포 주소에서 강력 새로고침 후 결과 확인
